...
#include "tensorflow/lite/experimental/shlo/ops/rsqrt.h"
...
template <>
struct ParamName<RsqrtOp> {
  static std::string Get() { return "rsqrt"; }
};
...
template <>
F16 Rsqrt::operator()<F16>(F16 val) const {
  return F16(operator()(static_cast<float>(val)));
}

template <>
BF16 Rsqrt::operator()<BF16>(BF16 val) const {
  return BF16(operator()(static_cast<float>(val)));
}

INSTANTIATE_TYPED_TEST_SUITE_P(Rsqrt, UnaryElementwiseOpShapePropagationTest,
                               RsqrtOp, TestParamNames);

INSTANTIATE_TYPED_TEST_SUITE_P(
    Rsqrt, UnaryElementwiseSameBaselineElementTypeConstraintTest,
    UnaryElementwiseConstraint1Types<RsqrtOp>, TestParamNames);

using UnsupportedTypes = WithOpTypes<
    RsqrtOp, ConcatTypes<BoolTestType, IntTestTypes, PerAxisQuantizedTestTypes>>;

INSTANTIATE_TYPED_TEST_SUITE_P(Rsqrt, UnaryElementwiseUnsupportedTypeTest,
                               UnsupportedTypes, TestParamNames);

template <class T>
struct FloatRsqrtTest : ::testing::Test {};

TYPED_TEST_SUITE(FloatRsqrtTest, FloatTestTypes, TestParamNames);

TYPED_TEST(FloatRsqrtTest, FloatTensorsWork) {
  ...
  absl::c_transform(input_data, expected_data.begin(), rsqrt_ref);

  auto op = Create(RsqrtOp::Attributes{});
...
template <class T>
struct QuantizedRsqrtTest : ::testing::Test {};

TYPED_TEST_SUITE(QuantizedRsqrtTest, QuantizedTestTypes, TestParamNames);

TYPED_TEST(QuantizedRsqrtTest, PerTensorWorks) {
  using StorageT = typename TypeParam::StorageT;
  ...
  absl::c_transform(
      input_data, expected_data.begin(), [zero_point, scale](auto v) {
        ...
        const ExpressedT dequantized_res = rsqrt_ref(dequantized_input);
